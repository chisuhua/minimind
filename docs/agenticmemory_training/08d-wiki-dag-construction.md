...
- **教师模型参数**(沿用 `08a §3`):DeepSeek V4 Flash 或等价教师 | 温度 T≤1.0 | 置信度用答案 span 平均 logprob | `top_logprobs=20` — 本文 Step 1-5 复用其输出
...
| 触发条件 | 回退动作 |
|---|---|
| Step 1-2 规范化后实体/关系数量下降 > 30% | 触发重试(重新规范化)或回退人工审核队列 |
| Step 3-5 任一 teacher 调用失败 ≥3 次 | 该页标记 `incomplete=true`(自定义字段,P1 验证时使用),跳过 Step 6 自评 |
| Step 6 `irr_estimate < 0.5` | 触发整页重生成(重新调用 teacher,Step 1-6 全部重跑) |
| Step 7 高置信推理链(`needs_reasoning_model_verification=false`)比例 > 80% | 提示 prompt 过于宽松,触发人工抽样核查(避免幻觉漏报) |
...
**循环依赖规避(2026-08-26 Oracle 评审发现)**:

Wiki DAG 构建阶段与 Schema 涌现阶段(08a Phase 3)在工程流水线上**可并行**,但 Wiki DAG 中的 `basic_info.type` / `core_facts.schema_ref` / `completeness_metadata.schemas_applied` 字段只接受**涌现 schema 已稳定后**(08a Phase 3 完成)的快照。**构建期缺失 schema 时,这些字段允许 `null`,在 P1 验证前用 08a Phase 3 输出回填**。

这是为什么 O1.3(层级推断)采用 "LLM 抽取 + 后置校验" 而非 "基于 schema" 的核心原因(避免循环依赖)。

**O1.1 默认建议补充**(2026-08-26):"参考 08a §4.2 L1-L3 语义匹配策略"(精确匹配/嵌入相似度/NLI 蕴含),**节点去重决策用本文档独立的混合方案**——粗筛用 `all-MiniLM-L6-v2` 嵌入聚类(min_cluster_size=10),精筛用 LLM 实体对齐调用。
...
| **O1.3** | 层级推断(is_a / part_of)算法 | (a) 基于涌现 schema(后置校验,见 C-1)<br/>(b) LLM 直接从原文抽取 is_a/part_of<br/>(c) 预定义 + LLM 修正 | **(b) LLM 抽取 + (a) 后置校验** | P1 启动前 |
| **O7** | `irr_estimate` 计算方法 | (a) 模型自评 (b) 教师评 (c) 综合(自评×权重+教师评×权重) | (c) 综合(教师评 0.6 + 自评 0.4,理由:教师覆盖度比小模型自评更可靠;保留 40% 自评以捕捉模型对自身边界的元认知信号,对应 `irr_estimate` 的能力自知语义) | P1 训练启动前 |
...
| **v0.2** | **2026-08-26** | **Oracle 评审修复** — 4 项修改见下表 | Sisyphus(AI 助手) |

### v0.2 变更明细(2026-08-26 Oracle 评审触发)

| # | 优先级 | 位置 | 变更 |
|---|------|------|------|
| 1 | 🔴 CRITICAL | §5 O1.3 | 默认建议由 "(a) 基于 schema" 改为 "(b) LLM 抽取 + (a) 后置校验",避免循环依赖 |
| 2 | 🔴 CRITICAL | §3.2 输入框 | 追加"教师模型参数(沿用 08a §3)"一行,消除 Step 1-5 teacher 来源假设 |
| 3 | 🟡 NICE-TO-HAVE | §5 O7 | 默认建议补充权重理由(教师覆盖度更可靠 + 保留自评捕捉元认知) |
| 4 | 🟡 NICE-TO-HAVE | §3.2 末尾 | 新增"失败回退策略"表(规范化下降 30% / teacher 失败 3 次 / irr_estimate < 0.5 / 高置信比例 > 80%) |
| 5 | 🔴 配套说明 | §3.3 表格下 | 新增"循环依赖规避"段落,说明 Wiki DAG 与 Schema 涌现的工程流水线并行 + 缺失 schema 时字段允许 null + O1.3 改方案的原因 |
| 6 | 🟡 配套说明 | §3.3 表格下 | 新增"O1.1 默认建议补充",明确节点去重用本文档独立的混合方案(`all-MiniLM-L6-v2` 粗筛 + LLM 精筛),不引用 08a §4.2 语义匹配 |
