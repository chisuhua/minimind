...
| 🆕 [`2026-arxiv-survey.md`](./2026-arxiv-survey.md) | SURVEY | **2026 arXiv 论文综合调研报告**(2026-08-30,9 节)——直接访问 arxiv.org 全文核验+深读 6 篇 2026 arXiv ID(Pandey RAG 利用率 / Wired Overconfidence 电路 / EST-PRM 压力测试 / Adaptive Test-Time Compute / Multi-Reward RLIF / RAG 表示分析),每篇产出"内容摘要+创新点+对项目借鉴"4 维分析(对 SOCA / architectures / AgenticDSL 训练链路各自的行动建议);**关键发现**:#1+#5 对 F-01 决策影响最大;**#3 直接支持否决 PRM 路线**;**#4 修正了原"DIPA"误标** |
...
| v1.7 | 2026-08-30 | **2026 arXiv 论文综合调研**([`./2026-arxiv-survey.md`](./2026-arxiv-survey.md),33 KB,9 节)——直接访问 arxiv.org 全文核验+深读 6 篇 2026 arXiv ID,产出每篇"内容摘要+创新点+对项目借鉴"4 维分析(对 SOCA / architectures / AgenticDSL 训练链路 各自的行动建议):① [2603.11513 Pandey et al.](https://arxiv.org/abs/2603.11513) RAG utilization bottleneck(≤7B 利用率 ≤14.6%,净增分 = 负);② [2604.01457 Wired for Overconfidence](https://arxiv.org/abs/2604.01457) (COLM 2026) verbalized confidence 电路定位(middle-to-late MLP blocks,激活 steering ECE 0.49→0.11);③ [2606.00437 EST-PRM](https://arxiv.org/abs/2606.00437) PRM stress-testing(支持否决 PRM 路线);④ [2604.21018 Adaptive Test-Time Compute](https://arxiv.org/abs/2604.21018)(**核验修正原"DIPA"误标**,原 99 §九 #25 与 00 §七 #24 同步修订);⑤ [2605.22620 Multi-Reward RLIF](https://arxiv.org/abs/2605.22620) 1.5B GSM8K 68%(multi-reward + GDPO + KL-Cov,与 F-03 RLIF 路线强相关);⑥ [2602.20091 How Retrieved Context Shapes](https://arxiv.org/abs/2602.20091) hidden states 锚定效应(1 relevant 文档锚定表示)+ late-layer param knowledge dominance(直接支撑 SOCA Bus 设计)。**关键发现**:#1+#5 对 F-01 决策影响最大;**#3 直接支持 architectures v3/v4 否决 PRM 路线的实证判断**;**#4 DIPA 误标**提醒文档持续做事实性错误审计。
| v1.8 | 2026-08-30 | **2026 arXiv 论文按问题领域拆分结构**——根据"论文要解决的问题"将 6 篇论文重组为 5 个独立子目录:① [`../retrieval-augmented-generation/`](../retrieval-augmented-generation/) (论文 #1 Pandey + #6 Yeh);② [`../confidence-calibration/`](../confidence-calibration/) (论文 #2 Zhao);③ [`../reward-model-robustness/`](../reward-model-robustness/) (论文 #3 Shihab);④ [`../test-time-compute/`](../test-time-compute/) (论文 #4 Zuo);⑤ [`../reinforcement-learning/`](../reinforcement-learning/) (论文 #5 Joarder)。每个目录含 README(领域定位+论文清单+跨论文对比+项目借鉴)+ 6 篇独立论文分析文件(每篇 150-250 行)。[`./2026-arxiv-survey.md`](./2026-arxiv-survey.md) 降级为**索引文档**(217 行 vs 原 594 行),提供全局视角+跨论文对比+决策矩阵。原 v1.0 单文件内容已完整拆分到 6 个独立文件,各 README 含交叉引用。**总计 1587 行**分布在 12 个文件中——文件数 +2x,行数 +2.7x,但每篇论文可单独引用且按问题领域组织,避免单文件过大淹没关键细节。 |
