...
> ⚠️ **修订 2026-08-30**：本节原表将两个不同实验条件（oracle utilization vs naive RAG distraction）的数据混入同一行,造成 1.5B 行"检索后损失 -57%"与"Oracle EM 10% - Known 100% = -90pp"不自洽。修订后:详细数据表移至 [`00-iteration-timeline.md` §4.5](./00-iteration-timeline.md)(两个子表 A/B 分别对应两实验条件),本节只保留关键数字与闭环影响。

**Pandey et al. 实证关键数字**(完整表格与口径说明见 00 §4.5):

| 条件 | SmolLM2-360M | Qwen2.5-1.5B | Qwen2.5-3B | Qwen2.5-7B |
|---|---|---|---|---|
| **A. Oracle 利用率**(答案 100% 在检索中) | 0% | **10%** | 12.8% | 14.6% |
| **B. Naive RAG 干扰损失**(加入普通检索后已知答案损失) | -100% | **-57%** | -45.6% | -41.6% |

**核心洞察**:
- **条件 A**:即使检索 100% 召回答案，1.5B 只能提取 10% 的答案——这是 **utilization 瓶颈**，不是 retrieval 瓶颈
- **条件 B**:任何检索（即使非 oracle）都会摧毁 42-100% 的"模型本来答对的"答案——**distraction effect**,这是 v4.6 hallucination amplification 的根源(详见 04b §2.6)
- **61-100% 的失败是"irrelevant generation"**——模型完全忽略提供的上下文

**对闭环的具体影响**:
- 闭环第 4 步 utilization 天花板 = 10%(条件 A)
- 闭环第 3 步检索副作用 = -57%(条件 B),即使 oracle 也无法避免
- 两者构成闭环的**双重硬天花板**——闭环 6% 端到端成功率(`100% × 0.7 × 0.7 × 0.12 ≈ 6%`)只是这两个数字的乘法累积,不是闭环本身的"缺陷"
- v4.7 元认知闭环启动后,唯一可改善的环节是置信度估计(第 2 步 70%→更高),但 utilization 与 distraction 都不会因此改善
...
- **每次重试的预算**（per-retry 上限,该次重试本身可消耗的时间）：
  - 第 1 次重试：可触发一次 retrieval，per-retry 预算 = 原始推理的 1.5x
  - 第 2 次重试：可触发一次 retrieval + 一次 verification，per-retry 预算 = 原始推理的 2.0x
  - 2 次重试仍失败：直接走 fallback（不进入第 3 次重试）

**成本/延迟控制硬上限**（**累计**口径,跨所有尝试）:
- **单 query 总耗时 ≤ 原始推理的 3x**(累计上限;若加入第 N 次重试将突破此上限,则跳过该重试直接 fallback)
- 单 query 额外 token 消耗 ≤ 原始的 2.5x
- 检索次数 ≤ 2（避免 GraphRAG 被当作 oracle）

> ⚠️ **修订 2026-08-30**:原 §5.3 存在算术含糊:
>
> **问题 1**:Per-retry 预算 vs 累计上限未区分。按字面"1 + 1.5 + 2.0 = 4.5x"超过"3x 累计上限",读者无法判断 4.5x 是否允许。
>
> **修订后语义**:
> - **per-retry 预算**:定义"这一次重试**最多**可用多少时间",不是"这一次重试**一定**花多少"。
> - **累计上限**:跨所有尝试(原始 + 重试)**总和**不超过 3x。
> - 若加入下一次重试将导致累计 > 3x,则**跳过该重试**,直接走 fallback。
>
> **示例**(原始推理 = 1.0x 时间单位):
> | 步骤 | 单次耗时 | 累计 | 是否触发 |
> |---|---|---|---|
> | 原始推理 | 1.0x | 1.0x | 必执行 |
> | 第 1 次重试 | ≤1.5x | ≤2.5x | 若置信度低,**仅当**累计+1.5x ≤3x 才触发 |
> | 第 2 次重试 | ≤2.0x | ≤4.5x | **仅当**累计+2.0x ≤3x 才触发;否则跳过 |
>
> **问题 2**:"第 3 次仍失败"措辞错误(max_retries=2 时不存在第 3 次重试)。修订为"2 次重试仍失败"。
